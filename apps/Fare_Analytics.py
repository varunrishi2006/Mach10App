...
initial_active_cell = {"row": 4, "column": 4, "column_id": "Total", "row_id": "Total"}
test_cols = ['Departure Status', 'Price Undercut', 'Competitively Priced',
             'Price Premium', 'Total']
...
def generate_control_card():
    ...
    return html.Div(
        ...
        children=[
            ...
            html.P(),
            html.P("Select Acceptable Price Variation Range (USD)"),
            dcc.RangeSlider(
                                ...
                                html.Div(
                                    [
                                        dash_table.DataTable(
                                            ...
                                            tooltip_header={
                                                'Price Undercut': 'Selling below Acceptable Price Variation',
                                                'Competitively Priced': 'Selling within Acceptable Price Variation',
                                                'Price Premium': 'Selling above Acceptable Price Variation  ',
                                            },
                                            tooltip_data=[
                                                {
                                                    'Departure Status': 'Departures below desired forecast',
                                                    'Price Undercut': 'Load critical departures selling below market price',
                                                    'Competitively Priced': 'Load critical departures selling at market price',
                                                    'Price Premium': 'Load critical departures selling above market price',
                                                },
                                                {
                                                    'Departure Status': 'Departures within desired forecast',
                                                    'Price Undercut': 'Competitive departures selling below market price',
                                                    'Competitively Priced': 'Competitive departures selling at market price',
                                                    'Price Premium': 'Competitive departures selling above market price',
                                                },
                                                {
                                                    'Departure Status': 'Departures above desired forecast',
                                                    'Price Undercut': 'Yield critical departures selling below market price ',
                                                    'Competitively Priced': 'Yield critical departures selling at market price',
                                                    'Price Premium': 'Yield critical departures selling above market price',
                                                }
                                            ],
                                            css=[{
                ...
                html.Div(
                    [
                        ...
                        html.Span(children=[
                            ...
                            html.P(),
                            html.I(
                                "* Apart from the forecast user can also identify the pricing status (Price Premium, Price Undercut or Competitively-priced), colors highlighted at the peak of a bar",
                                style={'fontSize': 14}),
                            html.P(),
                            html.I(
                ...
                html.Div(
                    [
                        ...
                        html.Span(children=[
                            ...
                            html.P(),
                            html.I(
                                "* Light Pink color on the bar is flagging departures 'selling higher than Industry Minimum' while light blue indicating 'below Industry Minimum'. ",
                                style={'fontSize': 14}),
                        ]),
...
def update_price_delta_matrix(cleaned_data, agg_method):
    ...
    # df_pivot.rename(index={'All': 'Total'}, inplace=True)
    # df_pivot.rename(columns={'All': 'Total'}, inplace=True)
...
def update_price_dist_ndo(cleaned_data, agg_method, selected_data):
    ...
    if selected_data is not None and selected_data['column_id'] != 'Departure Status':
        ...
        if selected_data['column_id'] == 'Total' and selected_data['row_id'] in (
        ...
        elif selected_data['row_id'] == 'Total' and selected_data['column_id'] in (
                ['Price Undercut', 'Competitively Priced', 'Price Premium']):
            df_final1 = df_final_updated[df_final_updated['Fare_Category'] == selected_data['column_id']]
            df_bar = df_final1.groupby(['NDO Range', 'Departure Status'])['Fare'].count()
        elif selected_data['row_id'] == 'Total' and selected_data['column_id'] == 'Total':
    ...
    df_bar_final.rename(columns={'NDO Range': 'Days-to-Departure Range'}, inplace=True)
    ...
    if agg_method == 'perc_count':
        ...
        df_bar_final.loc[:, "Departure Distribution (in %)"] = (df_bar_final['Departure Count'] / fare_count)
        df_bar_final_1 = df_bar_final.copy()
        df_bar_final_1['Departure Distribution (in %)'] = df_bar_final_1['Departure Distribution (in %)'] * 100

        fig = px.bar(df_bar_final_1, x='Days-to-Departure Range', y='Departure Distribution (in %)',
                     color='Departure Status',
                     barmode='stack', orientation='v', template='ggplot2', custom_data=['Departure Count'],
                     # text_auto=True
                     )
        dfs_1 = df_bar_final.groupby('Days-to-Departure Range')['Departure Distribution (in %)'].sum()
        df_6 = dfs_1.reset_index()
        df_6['Departure Distribution (in %)'] = df_6['Departure Distribution (in %)'].apply(
            lambda x: '{:.2%}'.format(x))
        fig.update_layout(
        ...
        fig.add_trace(go.Scatter(
            x=df_6['Days-to-Departure Range'],
            y=df_6['Departure Distribution (in %)'],
            text=df_6['Departure Distribution (in %)'],
            mode='text',
        ...
        fig = px.bar(df_bar_final,
                     x='Days-to-Departure Range',
                     y='Departure Count',
                     color='Departure Status',
                     barmode='stack',
                     orientation='v',
        ...
        dfs = df_bar_final.groupby('Days-to-Departure Range')['Departure Count'].sum()
        df_5 = dfs.reset_index()
        fig.update_layout(
            plot_bgcolor='rgba(0,0,0,0)',
            paper_bgcolor='rgba(0,0,0,0)',
            legend=dict(
                ...
                xanchor="right",
                x=0.8),
            yaxis=dict(tickvals=[10, 20, 50, 100, 150, 200])
        )

        fig.add_trace(go.Scatter(
            x=df_5['Days-to-Departure Range'],
            y=df_5['Departure Count'],
            text=df_5['Departure Count'],
            mode='text',
            textposition='top center',
            textfont=dict(
                size=14,
...
def update_forecast(selected_data, cleaned_data, forecast_range):
    forecast_lower_limit = forecast_range[0]
    forecast_upper_limit = forecast_range[1]
    datasets = json.loads(cleaned_data)
    dff = pd.read_json(datasets['df_3'], orient='split')
    ...
    fig = go.Figure()
    if selected_data is not None and selected_data['row_id'] != 'Total' and selected_data['column_id'] != 'Total':
        departure_status = selected_data['row_id']
        fare_category = selected_data['column_id']
        df_final1 = df_final_updated[(df_final_updated['Departure Status'] == departure_status) &
                                     (df_final_updated['Fare_Category'] == fare_category)]
    ...
    df_final_result['colors'] = df_final_result['Fare_Category'].apply(
        lambda x: 1 if x == 'Competitively Priced' else (2 if x == 'Price Undercut' else 3))
    # print(f'Check the value of colors {df_final_result.colors.unique()}')

    fig = px.bar(df_final_result, x='Departure Time', y='Forecast', color='Fare_Category')
    # Create the line annotation
    line_annotation_1 = go.layout.Shape(
        type='line',
        x0=min(df_final_result['Departure Time']),
        y0=forecast_lower_limit,
        x1=max(df_final_result['Departure Time']),
        y1=forecast_lower_limit,
        line=dict(color='red', width=3, dash='dot')
    )
    # Create the text annotation
    text_annotation_1 = go.layout.Annotation(
        x=min(df_final_result['Departure Time']),
        y=forecast_lower_limit + 3,
        text='Forecast lower limit',
        showarrow=False,
    ...
    line_annotation_2 = go.layout.Shape(
        type='line',
        x0=min(df_final_result['Departure Time']),
        y0=forecast_upper_limit,
        x1=max(df_final_result['Departure Time']),
        y1=forecast_upper_limit,
        line=dict(color='black', width=3, dash='dot')
    )

    # Create the text annotation
    text_annotation_2 = go.layout.Annotation(
        x=min(df_final_result['Departure Time']),
        y=forecast_upper_limit + 3,
        text='Forecast Upper limit',
        showarrow=False,
    ...
    fig.add_shape(line_annotation_1)
    fig.add_shape(line_annotation_2)
    # Add the text annotation to the figure
    fig.add_annotation(text_annotation_1)
    fig.add_annotation(text_annotation_2)
...
def update_industry_min(selected_data, cleaned_data):
    ...
    df_fare_comp_res['Below_Ind_Min'] = df_fare_comp_res.apply(
        lambda x: 1 if x['Industry Minimum'] > x['Selling Fare'] else 0, axis=1)
    df_fare_comp_res['Departure Time'] = pd.to_datetime(df_fare_comp_res['Departure Time'])
    color_mapping = {0: 'lightpink', 1: 'lightblue'}
    ...
    fig.add_trace(
        go.Bar(
            ...
            marker={'color': [color_mapping[val] for val in df_fare_comp_res['Below_Ind_Min']]},
    ...
    fig.update_xaxes(
        range=[min(df_fare_comp_res['Departure Time']) - padding, max(df_fare_comp_res['Departure Time']) + padding])

    fig.update_layout(
        # title="Industry Minimum Fare Comparison",
        xaxis_title="Departure Date",
        yaxis_title="Fares (USD)",
        title_x=0.5,
...
def update_fare_comp(selected_data, forecast_range, price_delta, dtd_range, market, sector, comp, flight_no,
    ...
    df_final1['Departure Time'] = pd.to_datetime(df_final1['Departure Time'])
    ...
    padding = timedelta(days=0.5)
    fig.update_xaxes(
        range=[min(df_client_aa['Departure Time']) - padding, max(df_client_aa['Departure Time']) + padding])
...
def comp_fare_frequency(dtd_range, market, sector, comp, flight_no, comp_flight):
    ...
    fig = px.bar(df_grouped_fares,
                 x='Selling Fare',
                 y='Departure_Count',
                 labels=dict(x="Selling Fare (USD)", y="Total Number of Fare Occurences", color="Carrier"),
                 color='Carrier',
                 barmode='group',
                 template='ggplot2'
                 )
